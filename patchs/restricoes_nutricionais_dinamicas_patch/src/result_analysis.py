...
def nutrient_result_display(constraints: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in constraints.iterrows():
        ...
        if original_status == "Sem restrição":
            visual_status = "⚪ Sem restrição"
        elif original_status == "Dados incompletos":
            visual_status = "⚫ Não calculado"
        elif original_status != "Atendida" or (nearest_margin is not None and nearest_margin < -1e-7):
