...
def numeric_series(values: pd.Series, default: float | None = None) -> pd.Series:
    """Converte uma Series para números respeitando formatos pt-BR e en-US.

    Diferentemente de ``pd.to_numeric``, esta função aceita textos como
    ``"48,76"`` e ``"1.234,56"`` porque reutiliza ``as_float``. Valores
    vazios ou inválidos permanecem como NaN quando ``default`` é ``None``.
    """
    return values.map(lambda value: as_float(value, default))
