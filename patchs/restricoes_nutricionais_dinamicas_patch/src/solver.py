...
def solve_least_cost(items: pd.DataFrame, constraints: pd.DataFrame) -> SolverResult:
    ...
    display_constraints = constraints.copy()
    if not display_constraints.empty:
        display_constraints["nutriente"] = display_constraints["nutriente"].astype(str).str.strip()
        display_constraints = display_constraints[display_constraints["nutriente"].ne("")]
        display_constraints = display_constraints.drop_duplicates(subset=["nutriente"], keep="first")

    for _, row in display_constraints.iterrows():
        code = str(row["nutriente"])
        if code not in NUTRIENT_BY_CODE:
            continue
        values = _nutrient_vector(work, code)
        minimum = as_float(row.get("minimo"))
        maximum = as_float(row.get("maximo"))
        restricted = minimum is not None or maximum is not None

        if not np.isfinite(values).all():
            achieved = None
            situation = "Dados incompletos"
        else:
