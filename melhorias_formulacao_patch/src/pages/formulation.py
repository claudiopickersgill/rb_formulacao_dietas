...
from ..utils import as_bool, as_float, dataframe_fingerprint, format_brl, new_id
...
def render(repository: Repository, user: User) -> None:
    ...
    with st.form(
        ...
        with st.expander("Conferir composição nutricional carregada", expanded=False):
            preview_columns = [
                "nome",
                *[code for code in DEFAULT_CONSTRAINTS if code in full_items.columns],
            ]
            if full_items.empty:
        ...
        st.caption(
            "Os valores desta área são enviados juntos ao clicar em **Calcular dieta**. "
            "Você pode usar Enter ou Tab para navegar entre as células sem disparar o cálculo."
        )
...
def _render_result(
    ...
    with tab2:
        ...
        n4.metric("Fora do limite", int(outside))
        ...
        st.caption(
            "Um nutriente é marcado como **próximo ao limite** quando a margem até o limite mais próximo "
