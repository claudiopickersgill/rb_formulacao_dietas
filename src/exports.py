...
from .utils import format_brl, safe_filename
...
def make_csv_bytes(items: pd.DataFrame) -> bytes:
    return _display_items(items).to_csv(index=False, sep=";", decimal=",").encode("utf-8-sig")
...
def make_excel_bytes(metadata: dict, items: pd.DataFrame, constraints: pd.DataFrame) -> bytes:
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        ws.write_number(10, 1, float(metadata.get("custo_kg", metadata.get("custo_per_kg", 0)) or 0), money_fmt)
    ...
    for _, row in items.iterrows():
        item_rows.append(
            [
                ...
                f"{float(row.get('inclusao_min', 0) or 0):.3f}%",
                f"{float(row.get('inclusao_max', 0) or 0):.3f}%",
                f"{float(row.get('inclusao_calculada', 0) or 0):.4f}%",
