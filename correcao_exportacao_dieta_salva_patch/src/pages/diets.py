...
from ..utils import as_float, format_brl, new_id
...
def render(repository: Repository, user: User) -> None:
    ...
    filtered = diets.copy()
    if "custo_kg" in filtered.columns:
        filtered["custo_kg"] = filtered["custo_kg"].map(lambda value: as_float(value))
    if query.strip():
    ...
    metadata = normalize_loaded_metadata(bundle["metadata"])
    items = normalize_loaded_items(bundle["items"])
    constraints = normalize_loaded_constraints(bundle["constraints"])
    bundle = {**bundle, "metadata": metadata, "items": items, "constraints": constraints}

    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Custo/kg", format_brl(metadata.get("custo_kg"), 4))
...
def _render_comparison(repository: Repository, diet_ids: list[str]) -> None:
    ...
    for diet_id in diet_ids:
        ...
        if not bundle:
            continue
        meta = normalize_loaded_metadata(bundle["metadata"])
        name = str(meta.get("nome") or diet_id)
        cost_rows.append(
            {
                "Dieta": name,
                "Custo/kg": as_float(meta.get("custo_kg")),
